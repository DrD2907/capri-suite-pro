...
exports.handler = async (event) => { try{ const url=new URL(event.rawUrl); const date=url.searchParams.get('date'); const sid=url.searchParams.get('service_id'); if(!date||!sid) return {statusCode:400, body: JSON.stringify({error:'date y service_id requeridos'})}; const svc=(await sbGet('/services',{ select:'*', id:`eq.${sid}`, limit:'1'}))[0]; if(!svc) return {statusCode:400, body: JSON.stringify({error:'Servicio no encontrado'})}; const dur=svc.duration_minutes; const {open,close}=parseHours(date); const openM=toMin(open), closeM=toMin(close), step=parseInt(process.env.SLOT_STEP||'15',10);
